...
const PLAYER_SKINS = {
  ...
  phoenix: {
    ...
    svg: `
      <svg viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
        <polygon points="30,5 15,45 30,35 45,45" fill="#ff6b6b" stroke="#ff9b4a" stroke-width="2"/>
        <path d="M15,45 Q5,30 25,25" fill="none" stroke="#ff9b4a" stroke-width="2"/>
        <path d="M45,45 Q55,30 35,25" fill="none" stroke="#ff9b4a" stroke-width="2"/>
        <circle cx="30" cy="20" r="4" fill="#ffdd57"/>
        <polygon points="25,40 30,55 35,40" fill="#ffdd57" opacity="0.9"/>
      </svg>`
  },
  stealth: {
    name: 'Stealth',
    ...
    svg: `
      <svg viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
        <polygon points="30,15 5,50 30,45 55,50" fill="#1f2937" stroke="#c084fc" stroke-width="2"/>
        <polygon points="30,25 20,45 30,42 40,45" fill="#c084fc" opacity="0.8"/>
        <line x1="30" y1="15" x2="30" y2="45" stroke="#9333ea" stroke-width="2"/>
  ...
  crystal: {
    ...
    svg: `
      <svg viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
        <polygon points="30,5 20,30 30,55 40,30" fill="#e0f2fe" stroke="#00ccff" stroke-width="2"/>
        <polygon points="20,30 10,45 25,40" fill="#e0f2fe" stroke="#00ccff" stroke-width="1.5"/>
        <polygon points="40,30 50,45 35,40" fill="#e0f2fe" stroke="#00ccff" stroke-width="1.5"/>
        <circle cx="30" cy="30" r="5" fill="#ffffff" filter="drop-shadow(0 0 5px #00ccff)"/>
      </svg>`
  }
};

let currentSkin = localStorage.getItem('spaceInvadersSkin') || 'classic';
if (!PLAYER_SKINS[currentSkin]) currentSkin = 'classic';
...
function createPlayerShip() {
  ...
  ship.innerHTML = PLAYER_SKINS[currentSkin].svg;
...
function initMenu() {
  ...
  // Populate grid
  Object.keys(PLAYER_SKINS).forEach(skinId => {
    const skin = PLAYER_SKINS[skinId];
    const card = document.createElement('div');
    card.className = `skin-card ${skinId === currentSkin ? 'selected' : ''}`;
    ...
    card.innerHTML = `
      ...
      <div class="skin-name" style="color: ${skin.color}; text-shadow: 0 0 8px ${skin.color}80">${skin.name}</div>
    `;
    ...
    card.addEventListener('click', () => {
      ...
      currentSkin = skinId;
      localStorage.setItem('spaceInvadersSkin', currentSkin);
      initAudio();
      playSound('powerup');
    });
